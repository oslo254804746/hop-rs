...
- 编辑资产复用创建表单字段并在当前列表内按需打开 Drawer；保存后保留标签与搜索 URL。
- 目标地址提供一键复制、成功反馈和无 Clipboard API 时的降级复制路径。
- 凭据创建与编辑改为按需 Drawer；认证方式只展示所需密钥字段，编辑时只显示加密值状态而不回显密钥。
- 凭据列表展示受影响资产；仍被资产引用的凭据在界面与服务端都禁止删除。
- Known Hosts 展示清单匹配与指纹复制，并通过风险说明、显式确认和服务端确认字段保护 TOFU 信任重置。
