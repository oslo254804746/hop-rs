...
use hop_core::{decrypt_envelope, Asset, AuthType, Credential, HopDb, MasterKey};
use russh::{
    client,
...
pub async fn connect_asset_shell(
    ...
    let addr = (asset.hostname.as_str(), asset.port as u16);
...
async fn authenticate(
    ...
    let auth_type = AuthType::try_from(credential.auth_type.as_str()).map_err(anyhow::Error::msg)?;
    match auth_type {
        AuthType::Password => {
            let password = decrypt_field(master_key, credential, "password", credential.password_enc.as_deref())?;
            let result = session
        ...
        AuthType::Key | AuthType::KeyWithPassphrase => {
            let private_key = decrypt_field(master_key, credential, "private_key", credential.private_key_enc.as_deref())?;
            let passphrase = match credential.passphrase_enc.as_deref() {
                Some(_) => Some(decrypt_field(master_key, credential, "passphrase", credential.passphrase_enc.as_deref())?),
                None => None,
            };
            let key = decode_secret_key(&private_key, passphrase.as_deref()).context("decode target private key")?;
...
fn decrypt_field(master_key: &MasterKey, credential: &Credential, field: &str, envelope: Option<&str>) -> Result<String> {
