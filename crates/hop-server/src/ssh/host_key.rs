...
use russh::keys::{key::safe_rng, load_secret_key, ssh_key::LineEnding, Algorithm, PrivateKey};
...
pub fn load_or_generate(path: &Path, key_type: &str) -> Result<PrivateKey> {
    ...
    let key =
        PrivateKey::random(&mut rng, Algorithm::Ed25519).context("generate ed25519 host key")?;
